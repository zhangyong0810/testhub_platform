        ...
        <div class="configs-grid">
          <div v-for="config in configs" :key="config.id" class="config-card">
            ...
            <div class="config-details">
              ...
              <!-- testcase-kit 继承配置 -->
              <div v-if="config.behavior_config && config.behavior_config.source" class="behavior-config-section">
                <div class="behavior-config-header">
                  <h4>📋 继承自 {{ config.behavior_config.source }} ({{ config.behavior_config.source_version }})</h4>
                </div>

                <!-- 格式规范 -->
                <div v-if="config.behavior_config.test_conventions" class="behavior-subsection">
                  <h5>📐 测试用例格式规范</h5>
                  <div class="sub-detail" v-if="config.behavior_config.test_conventions.priority_definitions">
                    <span class="sub-label">优先级:</span>
                    <span v-for="(p, k) in config.behavior_config.test_conventions.priority_definitions" :key="k" class="priority-tag" :class="'p'+k.toLowerCase()">{{ k }}: {{ p.name }}</span>
                  </div>
                  <div class="sub-detail" v-if="config.behavior_config.test_conventions.naming_rules">
                    <span class="sub-label">命名规则:</span>
                    <span>L2名词短语 / L3分组标签 / L4动词短语≤20字 / L5仅复杂场景</span>
                  </div>
                  <div class="sub-detail" v-if="config.behavior_config.test_conventions.excel_columns">
                    <span class="sub-label">Excel列:</span>
                    <span>{{ config.behavior_config.test_conventions.excel_columns.columns.length }}列 (模块→子模块→编号→测试点→端口→优先级→标题→前置→步骤→预期)</span>
                  </div>
                </div>

                <!-- 风险规则 -->
                <div v-if="config.behavior_config.risk_catalog && config.behavior_config.risk_catalog.rules" class="behavior-subsection">
                  <h5>⚠️ 风险检查规则 ({{ config.behavior_config.risk_catalog.rules.length }}条)</h5>
                  <div class="risk-tags">
                    <span v-for="rule in config.behavior_config.risk_catalog.rules.slice(0, 7)" :key="rule.id" class="risk-tag">{{ rule.label }}</span>
                    <span v-if="config.behavior_config.risk_catalog.rules.length > 7" class="risk-tag">+{{ config.behavior_config.risk_catalog.rules.length - 7 }}条</span>
                  </div>
                </div>

                <!-- 测试模式 -->
                <div v-if="config.behavior_config.test_pattern_catalog && config.behavior_config.test_pattern_catalog.patterns" class="behavior-subsection">
                  <h5>🔍 测试模式库 ({{ config.behavior_config.test_pattern_catalog.patterns.length }}个)</h5>
                  <div class="pattern-tags">
                    <span v-for="p in config.behavior_config.test_pattern_catalog.patterns.slice(0, 5)" :key="p.id" class="pattern-tag">{{ p.label }}</span>
                    <span v-if="config.behavior_config.test_pattern_catalog.patterns.length > 5" class="pattern-tag">+{{ config.behavior_config.test_pattern_catalog.patterns.length - 5 }}个</span>
                  </div>
                </div>

                <!-- 生成原则 -->
                <div v-if="config.behavior_config.generation_behavior_config && config.behavior_config.generation_behavior_config.core_principles" class="behavior-subsection">
                  <h5>🧠 生成核心原则</h5>
                  <ul class="principle-list">
                    <li v-for="(val, key) in config.behavior_config.generation_behavior_config.core_principles" :key="key" :title="val">{{ key.replace(/_/g, ' ') }}</li>
                  </ul>
                </div>
              </div>
...
/* testcase-kit behavior config styles */
...
.priority-tag.p0 { background: #ffebee; color: #c62828; }
.priority-tag.p1 { background: #fff3e0; color: #e65100; }
.priority-tag.p2 { background: #e8f5e9; color: #2e7d32; }

.risk-tags, .pattern-tags {
  display: flex;
  flex-wrap: wrap;
  gap: 4px;
}
